...
public protocol RepositoryGuidelineRepositoryProtocol: Sendable {
    ...
    /// 모든 AI 에이전트가 공유하는 공통 지침 파일의 전체 경로를 반환합니다.
    func commonGuidelineFilePath(localPath: String) -> String

    /// 공통 지침 내용을 읽습니다. 파일이 없으면 nil을 반환합니다.
    func loadCommonGuideline(localPath: String) -> String?

    /// 공통 지침을 저장(생성 또는 덮어쓰기)합니다.
    func saveCommonGuideline(localPath: String, content: String) throws

    /// 공통 지침 파일이 이미 존재하는지 확인합니다.
    func commonGuidelineExists(localPath: String) -> Bool
}
